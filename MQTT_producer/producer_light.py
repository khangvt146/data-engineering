...
def generate_light(num):
    while True:
        try:
            light = random.randint(0,100)
            
            msg = {
                "deviceId": "light_sensors_" + str(num),
                "timestamps": datetime.datetime.now().strftime("%Y-%m-%dT%H:%M:%S.%f%z"),
                "value": light,
                "unit": "lm"
            }
            
            print("Publish message with light: ", msg)
...
if __name__== '__main__':
    ...
    for i in range (0,_thread):
        t = threading.Thread(target=generate_light, args=(i,))
        t.start()
        thread_lst.append(t)
